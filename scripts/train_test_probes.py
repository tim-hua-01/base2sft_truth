...
def main():
    ...
    # Get datasets
    # Collect all individual dataset names needed, expanding combined tasks
    all_task_names = set()
    for task in list(train_task_list) + list(test_task_list):
        if task in COMBINED_TASKS_CONFIG:
            all_task_names.update(COMBINED_TASKS_CONFIG[task]['tasks'])
        else:
            all_task_names.add(task)
    all_datasets = utils.get_all_dataset(sorted(all_task_names), args.model_name)
